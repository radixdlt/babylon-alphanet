...
export default class EcdsaSecp256k1KeyPair implements KeyPair {
  ...
  sign(message: Uint8Array): Uint8Array {
    ...
    let doubleHash: Uint8Array = byteArrayFromHex(
      doubleHashWordArray.toString()
    );
  ...
  signHash(hash: Uint8Array): Uint8Array {
    ...
    if (hash.length !== 32) {
      throw new Error(
        "Only 32 byte long arrays can be signed. Consider using the `sign` func"
      );
    }
    let { signature, recid }: { signature: Uint8Array; recid: number } =
      secp256k1.ecdsaSign(hash, this.privateKey());
